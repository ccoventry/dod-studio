...
those files; see its README. On the 25th Anniversary `hw.dll` (#370) it swaps
map textures, model skins and sprites, capped at 512 per side, and keeps the
leftover-texture fix; detail textures, skies and the 1024 ceiling are
pre-Anniversary only so far (`tools/verify_texture_hires_offsets.py
--anniversary`).
