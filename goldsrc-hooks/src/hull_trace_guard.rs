...
//! off for a session.
//!
//! Analysis subject: the pre-Anniversary `hw.dll`, 1,641,376 bytes. The 25th
//! Anniversary `hw.dll` has the same bug in two differently compiled copies
//! (`+0x1e2540`, `+0x237540`); this signature doesn't match them, so there it
//! logs "not found" and does nothing. `tools/verify_hull_trace_offsets.py`
//! checks every fact above against the binary.
...
/// reproduces, then `mov eax, [ebp+0xc]` (the node number) and the leaf tests.
/// Unique in `hw.dll`.
const PATTERN: &str = "55 8B EC 83 EC 20 8B 45 0C 53 56 57 85 C0 7D 4F 83 F8 FE 74 34 \
                       8B 4D 20 83 F8 FF C7 01 00 00 00 00";

/// `push ebp; mov ebp, esp; sub esp, 0x20` -- the bytes the jump overwrites,
/// reproduced verbatim in the stub. Not relative, and none reads the flags.
const STOLEN: [u8; 6] = [0x55, 0x8b, 0xec, 0x83, 0xec, 0x20];
...
///         cmp  edx, MAX_PLANE
///         jae  .plane
/// .game:  push ebp; mov ebp, esp; sub esp, 0x20
...
///         ret
/// ```
fn stub(resume: usize, too_deep: usize, bad_node: usize, bad_plane: usize) -> Vec<u8> {
    const GAME: usize = 57;
    const DEEP: usize = 69;
    const NODE: usize = 78;
...
pub fn install() {
    ...
    match result {
        Ok(detour) => {
            ...
            unsafe {
                crate::debug::report(
                    "hull_trace_guard: PM_RecursiveHullCheck guarded against stale hulls (#384)",
                )
            };
        }
        Err(why) => unsafe {
...
fn install_at(base: usize) -> Result<detour::Detour, String> {
    // Safety: `base` is a module handle the loader gave us.
    let target = unsafe { scan::find_unique(base, PATTERN) }?;
    // Safety: the match is inside the code section and at least STOLEN long.
    ...
    // into it -- checked by tools/verify_hull_trace_offsets.py.
    unsafe { detour::install(target, STOLEN.len(), &code) }
}
...
pub fn status_line() -> Option<String> {
    ...
    Some(match refused() {
        0 => "hull-trace crash guard: on, nothing refused".to_string(),
        _ => format!("hull-trace crash guard: on, refused {}", breakdown()),
    })
}
...
mod tests {
    ...
    fn the_stub_assembles_to_what_the_comment_claims() {
        let code = stub(0x1111_1111, 0x2222_2222, 0x3333_3333, 0x4444_4444);
        #[rustfmt::skip]
        assert_eq!(
            code,
            vec![
    ...
    #[test]
    fn the_pattern_starts_with_the_stolen_bytes() {
        let head: Vec<u8> = PATTERN
            .split_whitespace()
            .take(STOLEN.len())
            .map(|t| u8::from_str_radix(t, 16).expect("no wildcards in the stolen bytes"))
            .collect();
        assert_eq!(head, STOLEN);
        assert!(STOLEN.len() >= 5, "a jmp rel32 needs 5 bytes");
    }
}
