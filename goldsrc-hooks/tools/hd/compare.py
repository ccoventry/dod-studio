...
usage: python compare.py <out.png> [<sample> ...]
...
import styles as S
from goldsrc import SPR_ADDITIVE, SPR_ALPHTEST, SPR_INDEXALPHA, SPR_NORMAL, Sprite, map_textures, mdl_textures
...
def main():
    if len(sys.argv) < 2:
        sys.exit(__doc__)
    out, specs = sys.argv[1], sys.argv[2:] or DEFAULTS
    hd = C.hd_dir()
    rows = []
    for spec in specs:
        try:
    ...
    for r, (kind, label, key, orig, fmt) in enumerate(rows):
        ...
        ref = next((Image.open(p).size for p in (os.path.join(hd, kind, s, key) for s in S.STYLES) if os.path.exists(p)),
                   None)
        if ref is None:
