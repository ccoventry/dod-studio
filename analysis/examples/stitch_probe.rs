...
fn main() {
    ...
    let input = a
        .next()
        .expect("usage: stitch_probe <in.dem> <head-secs> <tail-secs> <out.dem>");
    ...
    for (ei, entry) in demo.directory.entries.iter_mut().enumerate() {
        ...
        if !matches!(
            kept.last().map(|f| &f.frame_data),
            Some(FrameData::NextSection)
        ) && let Some(term) = entry
            .frames
            .iter()
            .rev()
            .find(|f| matches!(f.frame_data, FrameData::NextSection))
            .cloned()
        {
            kept.push(term);
        }
    ...
    let parsed = std::panic::catch_unwind(|| {
        open_demo_from_bytes(&out).map(|d| {
            d.directory
                .entries
                .iter()
                .map(|e| e.frames.len())
    ...
    std::panic::set_hook(Box::new(|_| {}));
    let an = std::panic::catch_unwind(|| {
        analysis::Analysis::try_from_bytes(&out).map(|a| {
            (
                a.state.initial_map_name.clone(),
                a.state.players.len(),
