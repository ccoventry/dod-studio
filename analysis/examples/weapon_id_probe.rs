    ...
    for entry in &demo.directory.entries {
        for frame in &entry.frames {
            ...
            for m in msgs {
                ...
                let Ok(UserMessage::DeathMsg(d)) = UserMessage::new(&um.name, &um.data) else {
                    continue;
                };
...
fn main() {
    let root = std::env::args()
        .nth(1)
        .expect("usage: weapon_id_probe <folder-or-demo>");
    ...
    println!(
        "scanned {read_ok}/{} demos, {total} DeathMsg records, {} distinct weapon IDs\n",
        demos.len(),
        hist.len()
    );
    ...
    for (id, (count, weapon)) in &hist {
        ...
        let note = if *weapon == Weapon::Unknown && *id != 0 {
            "  <-- UNMAPPED"
        } else {
            ""
        };
        println!(
            "{id:>4}  {count:>8}  {:>6.2}%  {weapon:?}{note}",
