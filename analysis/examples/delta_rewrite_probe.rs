...
fn main() {
    ...
    let input = args
        .next()
        .expect("usage: delta_rewrite_probe <in.dem> <out.dem>");
    ...
    for entry in demo.directory.entries.iter_mut() {
        for frame in entry.frames.iter_mut() {
            let FrameData::NetworkMessage(bt) = &mut frame.frame_data else {
                continue;
            };
    ...
    println!(
        "before: {:?}",
        before.iter().map(|v| format!("{v:.1}")).collect::<Vec<_>>()
    );
    println!(
        "after : {:?}",
        after.iter().map(|v| format!("{v:.1}")).collect::<Vec<_>>()
    );
    let ok = before.len() == after.len()
        && before
            .iter()
            .zip(&after)
            .all(|(b, a)| (a - b - SHIFT).abs() < 0.75);
    println!(
        "shift preserved: {}",
        if ok {
            "YES"
        } else {
            "NO (encoding is lossy or bit-packed against a reference)"
        }
    );
