...
fn main() {
    let path = std::env::args().nth(1).expect("usage: statusicon_probe <demo>");
            ...
            for m in msgs {
                match m {
                    ...
                    NetMessage::UserMessage(um) => {
                        ...
                        let data = &um.data;
                        let Some(&enable) = data.first() else { continue };
                        let enable = enable != 0;
                        let Some(nul) = data[1..].iter().position(|&b| b == 0) else { continue };
    ...
    println!("{:>12}  {:<8} {:<20} rgb", "viewdemo (s)", "action", "icon name");
    for (offset, enable, name, rgb) in &hits {
        let action = if *enable { "enable" } else { "disable" };
        let rgb_s = rgb.map(|(r, g, b)| format!("({r},{g},{b})")).unwrap_or_default();
