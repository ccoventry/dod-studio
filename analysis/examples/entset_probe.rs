...
fn main() {
            ...
            for m in msgs {
                ...
                match &**em {
                    EngineMessage::SvcPacketEntities(pe) => {
                        ...
                        if f.time <= join {
                            before_join.push(pe.entity_count.to_u32());
                        }
    ...
    println!(
        "last 6 entity_count values before the join: {:?}",
    ...
    if !per_packet.is_empty() {
        println!(
            "entities listed per packet: median {}, max {}",
            per_packet[per_packet.len() / 2],
            per_packet[per_packet.len() - 1]
        );
        println!(
            "entity_count field:         median {}, max {}",
