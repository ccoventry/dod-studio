...
fn main() {
    let path = std::env::args()
        .nth(1)
        .expect("usage: bridge_ceiling <demo.dem>");
    ...
    println!(
        "{} -- {:.1} MB, frame area {DEMO_HEADER_SIZE}..{end}",
        path,
    ...
    println!(
        "  {} of the {} bytes in the frame area were walked",
        stop - DEMO_HEADER_SIZE,
        end - DEMO_HEADER_SIZE
    );
    ...
    for t in 0..11 {
        ...
        println!(
            "  type {name:>3}: {:>8} frames, {:>12} bytes, {avg:>10.0} bytes/frame",
            w.hist[t], w.spans[t]
        );
    }
    println!(
        "  overall: {:.0} bytes/frame",
        (stop - DEMO_HEADER_SIZE) as f64 / frames.max(1) as f64
    );
    println!(
        "  time {:.2}s -> {:.2}s, {} frames stepped backwards in time",
        w.first_time, w.last_time, w.backwards
    );

    println!(
        "\nframes that swallowed 4 KB+ (each is a skip over rubble): {}",
        w.swallows.len()
    );
    let swallowed: usize = w.swallows.iter().map(|s| s.2).sum();
    if !w.swallows.is_empty() {
        println!(
            "  {swallowed} bytes ({:.0}% of the frame area) vanished this way",
        ...
        if w.swallows.len() > 12 {
            println!("    ... and {} more", w.swallows.len() - 12);
        }
    }
    ...
    match dem::open_demo_from_bytes(&bytes) {
        Ok(d) => {
            ...
            println!(
                "\ndem parse: {total} frames across {} entries",
                d.directory.entries.len()
            );
            for (i, e) in d.directory.entries.iter().enumerate() {
                println!(
                    "  entry {i} type {}: {} frames, frame_offset {}, length {}",
                    e.type_,
                    e.frames.len(),
                    e.frame_offset,
                    e.file_length
