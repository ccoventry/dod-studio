...
fn main() {
    ...
    let path = a
        .next()
        .expect("usage: deathmsg_diag <demo.dem> <t_lo> <t_hi>");
            ...
            for m in msgs {
                    ...
                    && let dem::types::EngineMessage::SvcUpdateUserInfo(ui) = &**em
                {
                    ...
                    let parts: Vec<&str> = raw
                        ...
                        .split('\\')
                        .collect();
                    let fmap: HashMap<&str, &str> = parts
                        .as_chunks::<2>()
                        .0
                        .iter()
                        .map(|c| (c[0], c[1]))
                ...
                if let NetMessage::UserMessage(um) = m {
                    ...
                    let name_str = String::from_utf8_lossy(&um.name)
                        .trim_matches('\0')
                        .to_string();
                    if name_str != "DeathMsg" {
                        continue;
                    }
                    match UserMessage::new(&um.name, &um.data) {
                        Ok(UserMessage::DeathMsg(d)) => {
                            ...
                            let killer_fixed =
                                names.get(&k_raw).cloned().unwrap_or(format!("#{k_raw}"));
                            let victim_fixed =
                                names.get(&v_raw).cloned().unwrap_or(format!("#{v_raw}"));
                            let killer_unfixed = names
                                .get(&d.killer_client_index)
                                .cloned()
                                .unwrap_or(format!("#{}", d.killer_client_index));
                            let victim_unfixed = names
                                .get(&d.victim_client_index)
                                .cloned()
                                .unwrap_or(format!("#{}", d.victim_client_index));
                            println!(
                                "frame_idx={idx} t={time:.2}s id={} OK: [-1 fixed] {killer_fixed} killed {victim_fixed} | [unfixed] {killer_unfixed} killed {victim_unfixed} with {:?} raw={:?}",
                                um.id, d.weapon, um.data
                            );
                        }
                        Ok(other) => {
                            println!(
                                "frame_idx={idx} t={time:.2}s id={} name matched DeathMsg but decoded as a DIFFERENT variant: {other:?} raw={:?}",
                                um.id, um.data
                            );
                        }
                        Err(_) => {
                            println!(
                                "frame_idx={idx} t={time:.2}s id={} PARSE ERRORED raw={:?} name_bytes={:?}",
