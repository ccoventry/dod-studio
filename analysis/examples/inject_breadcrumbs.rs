...
fn main() {
    ...
    let input = a
        .next()
        .expect("usage: inject_breadcrumbs <in.dem> <out.dem> [interval_secs]");
    let output = a.next().expect("out.dem");
    let interval: f32 = a
        .next()
        .map(|s| s.parse().expect("interval_secs"))
        .unwrap_or(5.0);
    ...
    println!(
        "injected {n} breadcrumbs every {interval:.1}s across {first_time:.1}s..{last_time:.1}s"
    );
