...
fn entry0_end(bytes: &[u8], end: usize) -> Option<usize> {
    ...
    for (i, &o) in offs.iter().enumerate() {
        if bytes[o] == 5 {
            return Some(offs.get(i + 1).copied().unwrap_or(o + FRAME_HEADER_SIZE));
        }
    }
    None
}
...
fn main() {
    ...
    println!(
        "entry0 ends {e0}; prefix ends {damage}; tail has {} frame starts mapped",
        tail.len()
    );
    ...
    for &skip in &[0usize, 1, 2, 3, 5, 10, 20, 50, 100] {
        let Some(&from) = tail.get(skip) else {
            continue;
        };
        ...
        let r = std::panic::catch_unwind(|| {
            dem::open_demo_from_bytes(&out).map(|d| {
                d.directory
                    .entries
                    .iter()
                    .map(|e| e.frames.len())
        ...
        if verdict.starts_with("PARSES") && best.is_none() {
            best = Some((skip, out));
        }
        std::panic::set_hook(Box::new(|_| {}));
        if best.is_some() {
