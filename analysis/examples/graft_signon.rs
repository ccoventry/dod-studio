...
fn step(bytes: &[u8], pos: usize, end: usize) -> Option<(u8, usize)> {
    ...
    match t {
        5 | 2 | 255 => {}
        0 | 1 => {
            if p + NETWORK_HEADER_ALIGNMENT > end {
                return None;
            }
            let l = i32::from_le_bytes(
                bytes[p + NETMSG_INFO_SIZE..p + NETWORK_HEADER_ALIGNMENT]
                    .try_into()
                    .ok()?,
            );
        ...
        8 => {
            ...
            let l = u32::from_le_bytes(bytes[p + 4..p + 8].try_into().ok()?) as usize;
            p += 24 + l;
        }
        9 => {
            if p + 4 > end {
            ...
            let l = u32::from_le_bytes(bytes[p..p + 4].try_into().ok()?) as usize;
            p += 4 + l;
        }
        _ => return None,
    }
...
fn main() {
    ...
    let bad_path = a
        .next()
        .expect("usage: graft_signon <damaged.dem> <donor.dem> <out.dem>");
    ...
    println!(
        "damaged: {bmap} ({bdir}) crc 0x{:08x}, protocols {bd}/{bn}",
        bcrc as u32
    );
    println!(
        "donor:   {dmap} ({ddir}) crc 0x{:08x}, protocols {dd}/{dn}",
        dcrc as u32
    );
    ...
    println!(
        "  damaged signon is {} bytes, donor signon is {} bytes",
        bad_signon - DEMO_HEADER_SIZE,
        donor_signon - DEMO_HEADER_SIZE
    );
    ...
    dir_entry(
        ...
        (playback_at - DEMO_HEADER_SIZE) as i32,
    );
    dir_entry(
        &mut out,
        1,
    ...
    match dem::open_demo_from_bytes(&out) {
        Ok(d) => println!(
            "  parse OK: {} frames across {} entries",
            d.directory
                .entries
                .iter()
                .map(|e| e.frames.len())
