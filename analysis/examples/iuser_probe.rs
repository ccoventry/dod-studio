...
fn main() {
    for path in std::env::args().skip(1) {
                        ...
                        EngineMessage::SvcClientData(cd) => {
                            for (k, v) in cd.client_data.iter() {
                                ...
                                if kk.starts_with("iuser")
                                    && let Some(i) = as_i32(v)
                                {
                                    *cd_iuser.entry(kk).or_default().entry(i).or_insert(0) += 1;
                                }
                            }
                        }
        ...
        println!("\n{path}");
        if cd_iuser.is_empty() {
            println!("  clientdata iuser*: absent");
        }
        for (k, vals) in &cd_iuser {
            ...
            println!(
                ...
                v.iter()
                    .take(8)
                    .map(|(val, n)| format!("{val}x{n}"))
                    .collect::<Vec<_>>()
            );
        }
        if per_entity.is_empty() {
            println!("  entity iuser*: ABSENT on every entity");
        }
