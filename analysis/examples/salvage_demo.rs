...
fn walk(bytes: &[u8], end: usize) -> Walk {
    ...
    loop {
        ...
        if pos + FRAME_HEADER_SIZE > end {
            ...
            w.reason = "reached end of file".into();
            return w;
        }
        ...
        match type_byte {
            ...
            0 | 1 => {
                if pos + NETWORK_HEADER_ALIGNMENT > end {
                    w.good_end = frame_start;
                    w.reason = "truncated network frame".into();
                    return w;
                }
                ...
                if len > MAX_PAYLOAD_LIMIT_BYTES {
                    w.good_end = frame_start;
                    w.reason = format!("impossible packet size {len}");
                    return w;
                }
            ...
            9 => {
                if pos + 4 > end {
                    w.good_end = frame_start;
                    w.reason = "truncated buffer".into();
                    return w;
                }
            ...
            _ => {
                w.good_end = frame_start;
                w.reason = format!("unhandled type {type_byte}");
                return w;
            }
        }
        if pos > end {
            w.good_end = frame_start;
            w.reason = "frame ran past end".into();
            return w;
        }
        w.good_end = pos;
    }
...
fn dir_entry(
    ...
    put_i32(out, type_);
    let mut d = [0u8; 64];
    for (i, b) in desc.bytes().take(63).enumerate() {
        d[i] = b;
    }
...
fn main() {
    ...
    println!(
        "walk: {} frames, last good time {:.2}s",
        w.frames, w.last_time
    );
    println!(
        "  stopped at byte {} ({:.1}% of frame area) -- {}",
        w.good_end,
        100.0 * (w.good_end - DEMO_HEADER_SIZE) as f64 / (end - DEMO_HEADER_SIZE).max(1) as f64,
        w.reason
    );
    let Some(entry0_end) = w.entry0_end else {
        println!("  no entry-0 boundary found; nothing to salvage");
        return;
    };
    ...
    dir_entry(
        ...
        (entry0_end - DEMO_HEADER_SIZE) as i32,
    );
    ...
    out[DIRECTORY_OFFSET_POS..DEMO_HEADER_SIZE]
        .copy_from_slice(&(new_dir_off as i32).to_le_bytes());

    std::fs::write(&output, &out).expect("write");
    println!(
        "  wrote {} ({:.1} MB, {:.1}% of original)",
        output,
        out.len() as f64 / 1e6,
    ...
    match analysis::Analysis::try_from_bytes(&out) {
        Ok(a) => println!(
            "  ANALYSIS OK: map={:?} players={} duration={:.1}s",
