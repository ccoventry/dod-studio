...
use native::patch::{StreamPatcher, build_batch_queue};

fn main() {
    let path = std::path::Path::new("local/demos/wsod25-grp_r1-dyelife_gskill_armory_h1.dem");
    let _rules = HighlightRules { max_time_gap: None };
    match scan_demo_for_highlights(path) {
        Ok((
            ...
            for (i, streak) in streaks.iter().enumerate() {
                println!(
                    "Streak {}: start_tick = {}, end_tick = {}, kills = {}",
                    i, streak.start_tick, streak.end_tick, streak.kill_count
                );
            }
            ...
            let global_arrays: std::collections::HashMap<
                std::path::PathBuf,
