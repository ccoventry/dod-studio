//! The HD Textures page's backend (#372): what is built, fetching the
//! upscaler (and, when the PC has none, a Python), and running the build.
...
use native::hd::{self, HdStatus, python, setup::SetupOutcome};
...
/// What is built under `<game>\dod\dodstudio_hd`, whether the upscaler and
/// its models are downloaded, and which Python a build would use.
/// `game_path` is the `hl.exe` the Configuration page holds.
#[tauri::command]
pub async fn hd_status(app: AppHandle, game_path: String) -> Result<HdStatus, String> {
    ...
    crate::messages::spawn_blocking_result(tokio::task::spawn_blocking(move || {
        ...
        let mut status = hd::scan(&root, &hd::setup::realesrgan_dir(&hd_tools));
...
/// skipping what is already there, plus the app's own Python when the PC
/// has none the build can use. Emits `hd_setup_progress` at most ~30 times a
/// second.
#[tauri::command]
pub async fn hd_setup_tools(
    ...
    let result = crate::messages::flatten_spawn_blocking(tokio::task::spawn_blocking(move || {
        ...
        hd::setup::run(&hd_tools, need_python, &cancel, &mut |progress| {
