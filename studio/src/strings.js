...
export const STRINGS = {
  ...
  HD: {
    ...
    MAPS_ADD_LABEL: 'Add a map or a pattern:',
    MAPS_ADD_PLACEHOLDER: 'dod_railroad* or dod_anzio',
    MAPS_ADD_BUTTON: 'Add',
    MAPS_PATTERN_HELP: '* matches any run of characters, so dod_railroad* is every railroad map; ? matches one character.',
    patternMatches: (count, names) => (count
      ? `Matches ${count} map${count === 1 ? '' : 's'}: ${names.slice(0, 6).join(', ')}${count > 6 ? `, and ${count - 6} more` : ''}.`
      : 'Matches none of your maps.'),
